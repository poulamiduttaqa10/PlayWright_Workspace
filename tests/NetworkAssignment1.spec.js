...
const loginAndGoToEvents = async (page) => {
    ...
    await page.locator("h1:has-text('Upcoming Events')").waitFor();
...
test("@API Test 2 — Banner is NOT visible when 4 events are returned", async ({page})=>{
    ...
    const eventCards = page.locator("[data-testid='event-card']");
    await expect(await eventCards.first()).toBeVisible();
    await expect(await eventCards).toHaveCount(4);
    const banner = page.getByText(/sandbox holds up to/i);
